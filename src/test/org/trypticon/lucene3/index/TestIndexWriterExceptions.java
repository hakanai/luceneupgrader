...
import org.trypticon.lucene3.analysis.CannedTokenStream;
import org.trypticon.lucene3.analysis.MockAnalyzer;
import org.trypticon.lucene3.analysis.MockTokenizer;
import org.trypticon.lucene3.analysis.Token;
import org.trypticon.lucene3.analysis.TokenFilter;
...
import org.trypticon.lucene3.search.PhraseQuery;
...
import org.trypticon.lucene3.store.MockDirectoryWrapper;
import org.trypticon.lucene3.store.RAMDirectory;
...
public class TestIndexWriterExceptions extends LuceneTestCase {
    ...
    public void eval(MockDirectoryWrapper dir)  throws IOException {
      if (doFail) {
        ...
        for (int i = 0; i < trace.length; i++) {
          if ("org.trypticon.lucene3.index.FreqProxTermsWriter".equals(trace[i].getClassName()) && "appendPostings".equals(trace[i].getMethodName()))
            sawAppend = true;
  ...
  private static class FailOnlyInSync extends MockDirectoryWrapper.Failure {
    ...
    public void eval(MockDirectoryWrapper dir)  throws IOException {
      if (doFail) {
        StackTraceElement[] trace = new Exception().getStackTrace();
        for (int i = 0; i < trace.length; i++) {
          if (doFail && "org.trypticon.lucene3.store.MockDirectoryWrapper".equals(trace[i].getClassName()) && "sync".equals(trace[i].getMethodName())) {
            didFail = true;
  ...
  private static class FailOnlyInCommit extends MockDirectoryWrapper.Failure {
    ...
    public void eval(MockDirectoryWrapper dir)  throws IOException {
      ...
      for (int i = 0; i < trace.length; i++) {
        if ("org.trypticon.lucene3.index.SegmentInfos".equals(trace[i].getClassName()) && "prepareCommit".equals(trace[i].getMethodName()))
          isCommit = true;
        if ("org.trypticon.lucene3.store.MockDirectoryWrapper".equals(trace[i].getClassName()) && "deleteFile".equals(trace[i].getMethodName()))
          isDelete = true;
      }
