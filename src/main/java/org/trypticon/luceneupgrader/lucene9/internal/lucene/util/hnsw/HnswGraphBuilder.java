...
import org.trypticon.luceneupgrader.lucene9.internal.lucene.util.hnsw.HnswUtil.Component;
    ...
    for (int i = 0; i < candidates.size(); i++) {
      ...
      if (hnswLock != null) {
        try (HnswLock.LockedRow rowLock = hnswLock.write(level, nbr)) {
          ...
          nbrsOfNbr.addAndEnsureDiversity(node, candidates.getScores(i), nbr, scorerSupplier);
        }
      } else {
